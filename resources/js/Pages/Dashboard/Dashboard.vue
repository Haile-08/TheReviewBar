...
const submitPost = () => {
    if (!description.value || !selectedMovie.value) return;
    
    router.post('/posts', {
        description: description.value,
        movie_id: selectedMovie.value.id,
        movie_title: selectedMovie.value.title || selectedMovie.value.name,
        movie_poster: selectedMovie.value.poster_path,
        release_date: selectedMovie.value.release_date
    });
};
</script>
...
<template>
    ...
    <div class="min-h-screen flex flex-col w-full bg-background font-sans text-foreground">
        <div class="w-full h-12 items-center px-10 flex justify-between bg-primary text-primary-foreground">
            ...
            <div class="flex items-center gap-4">
                ...
                <Link method="post" as="button" :href="route('logout')" class="font-bold hover:underline">Logout</Link>
            </div>
        </div>

        <div class="px-4 py-10 w-full max-w-4xl mx-auto flex flex-col items-center">
            
            <Card class="w-full max-w-2xl shadow-lg border-muted">
                <CardHeader class="flex flex-row items-center justify-between border-b pb-4 pt-4 px-6 bg-card">
                ...
                <CardContent class="p-6">
                    <div class="flex gap-4 mb-6">
                        <Avatar class="h-10 w-10 mt-1">
                            <AvatarImage :src="'/storage/' + user.profile" alt="User" />
                            <AvatarFallback>{{ user.name.charAt(0) }}</AvatarFallback>
                        </Avatar>
                        <Textarea
                            v-model="description"
                    ...
                    <div v-if="selectedMovie" class="relative bg-secondary/50 border rounded-xl p-3 flex items-center justify-between transition-all group">
                        <div class="flex items-center gap-4">
                            <img
                            ...
                            />
                            <div v-else class="w-14 h-20 bg-muted flex flex-col items-center justify-center rounded shadow-sm text-muted-foreground">
                    ...
                    <div v-else class="relative space-y-4">
                        ...
                        <div v-if="isLoading" class="text-primary text-sm font-medium animate-pulse ml-1">
                            Searching...
                        </div>

                        <ScrollArea v-if="results.length && !isLoading" class="h-[280px] rounded-md">
                            <div class="flex flex-col gap-2 pr-4">
                                <p class="text-xs font-bold text-muted-foreground uppercase tracking-wider mb-2 ml-1">Suggested Movies</p>
                                ...
                                >
                                    ...
                                    />
                                    <div v-else class="w-12 h-16 bg-muted flex items-center justify-center rounded-md text-muted-foreground">
                                        <ImageIcon class="h-5 w-5 opacity-50" />
                                    </div>

                                    <div class="flex-1">
                                        <h3 class="font-bold text-foreground">{{ item.title || item.name }}</h3>
                        ...
                        <div v-else-if="searchQuery && !isLoading && !results.length" class="text-muted-foreground mt-4 ml-1">
                            No movies found for "{{ searchQuery }}"
                        </div>
                    </div>
                </CardContent>

                <CardFooter class="bg-muted/30 px-6 py-4 flex justify-end gap-3 border-t">
                    <Button variant="ghost" class="text-muted-foreground hover:text-foreground">
                        Cancel
                    </Button>
                    <Button
                        @click="submitPost"
                        :disabled="!description || !selectedMovie"
            ...
            <div v-if="posts && Object.keys(posts).length" class="w-full max-w-2xl mt-12 flex flex-col gap-6">
                <Card v-for="post in posts" :key="post.id" class="overflow-hidden">
                    <CardHeader class="pb-2">
                        <div class="flex justify-between items-center">
                            <CardTitle class="text-base font-bold">{{ post.name }}</CardTitle>
                            <span class="text-sm text-muted-foreground">{{ post.created_at_human }}</span>
                        </div>
                    </CardHeader>
                    <CardContent>
                        <p class="text-foreground mb-4">{{ post.description }}</p>
                        
                        <div class="border rounded-lg overflow-hidden flex bg-muted/20">
                            <img v-if="post.poster" :src="post.poster" :alt="post.movie" class="w-24 object-cover" />
